...
Be sure to check the CSV parameters in the report design and follow the steps exactly from [Adding CSV Format to Reports](./advanced/csv-reports.md). Most likely, a column is not properly defined in the parameter according to the RDO column label.

If you have verified these items, please open a case with Support.

## Why can I not edit a "Read-Only" report?

Any Read-Only report, whether Standard or Custom, is a report that was developed by Engineering or Professional Services outside of BIRT Studio. Support and customers do not have access to this tool and therefore these reports cannot be edited. If you have a Read-Only report that needs to be edited, please open a Service Request.
...
The BIRT Studio that is within WFM is not the same BIRT Studio that comes out of the box. The product within WFM has been heavily customized to tailor it to the needs of the business and to fit within the current infrastructure. Some features do not work as they might if this was being used as standalone reporting software outside of WFM. The "Select Values" feature is one of them.
...
The "Render report failed" error will only occur when there is a column binding inside the report design, but its corresponding link has been deleted from the Report Data Object. If you have edited a report design recently and remember what column was deleted, adding it back to the RDO and saving will resolve the issue.

If you are not the one who made the change or do not remember what column was deleted, please reach out to Support who can reproduce the issue and review logs to determine the column that was deleted.
...
Answering this question simply in an FAQ is not possible. There is no single answer that will suit all use cases. For that reason, please refer to the [Troubleshooting](./troubleshooting/) pages for more information.

## I'm a manager with access to the entire organization. Why can I not see anyone else's reports they have run?

This is by design. Any manager who has access to the Report Library will only be able to see the reports that have been _run by_ them. This includes scheduled reporting jobs that designate the manager as the "Run As" user.
...
This pop-up is not an error and is common during the report design process. In BIRT Studio, the default parameters are today–today for All Home Locations (not All Home). This pop-up can simply mean that the conditions set in the report design for those default parameters return nothing. However, when run for a different set of parameters, the report will return data. Test the report by previewing it for a wider timeframe and All Home Locations (or a smaller one if All Home Locations is too large).

## How do I make **\_** report?

While we aim to provide the best documentation possible on reporting, BIRT Studio, and Dataviews, we are not able to create custom-tailored articles for every different kind of report. Please see the [cookbook](/docs/birt-studio/report-cookbook) for basic recipes on getting started creating specific kinds of reports. **These are not full how-to articles**.

We also have a few articles on creating specific types of reports such as "Hours by Paycode" in the [Community](https://community.ukg.com). There is also a Community article that contains several reporting templates created by reporting subject matter experts within Support. The files will be `.zip` files that can be downloaded and imported using Setup Data Manager (SDM).
...
For more information on this error and how to troubleshoot, please see the [Troubleshooting](./troubleshooting/) page.
